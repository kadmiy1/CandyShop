...
<script>
    ...
    var str = '<tr><td><select class="select-chosen"  name="candy[]">'+
    '<?php 
        // echo "<option value=0>Выберите сладость</option>";
    for ($i = 0 ; $i < $rows1 ; ++$i)
    {
        $row = mysqli_fetch_row($result);
        for ($j = 0 ; $j < 1 ; ++$j) echo "<option value=$row[0]>$row[1] -  $row[2] р/кг</option>";   
        ;
    } 
    ?></td>'+
    '<td><input class="inputadd" type="text" placeholder="Киллограммы" onchange="getmass(this);" name="mass[]" id="massa"></td>'+
    '<td id = "result">0</td>'+
    ...
    $('#add_1').click(function(){
        ...
        $(function select(){
            $('.select-chosen').chosen({
                width: 200,
                no_results_text: "Нет такой сладости!",
                placeholder_text_single: "Выберите сладость",
                search_contains: true,
                max_shown_results: 10
            });
        })
